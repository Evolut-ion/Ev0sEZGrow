    ...
    fun run() {
        ...
        if (!cachedJar.exists()) {
            ...
            try {
                ...
                println("ERROR: Failed to download server JAR: ${e.message}")
                return
            }
        } else {
        ...
        Thread { process.inputStream.bufferedReader().useLines { lines -> lines.forEach { println(it) } } }.start()
        Thread { process.errorStream.bufferedReader().useLines { lines -> lines.forEach { System.err.println(it) } } }.start()
        Thread { System.`in`.bufferedReader().useLines { lines -> lines.forEach { process.outputStream.write((it + "\n").toByteArray()); process.outputStream.flush() } } }.start()
